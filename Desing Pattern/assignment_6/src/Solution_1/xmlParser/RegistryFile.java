...
public class RegistryFile extends XMLFiles {
    ...
    // SINGLETON CLASS
    // --------------------------------------------
    private static RegistryFile instance;

    private RegistryFile(String path) {
        super(path);
    }

    public static RegistryFile getInstance(String path) {
        System.out.println("CHECK REGISTRY STATUS: " + instance);

        if (instance == null) {
            instance = new RegistryFile(path);
        }
