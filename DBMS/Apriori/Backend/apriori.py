...
stock = [('BISCUIT', 5), ('BOURNVITA', 6), ('BREAD', 4), ('COCK', 2), ('COFFEE', 9),
         ('CORNFLAKES', 5), ('JAM', 9), ('MAGGI', 4), ('MILK', 5), ('SUGER', 3), ('TEA', 7)]
...
def addData(data: any):
    ...
    # add value to transactions list
...
def initialize(transaction, support_vector):
...
frozenlistdata = aprioriAlgorithm(transactions, support_vector=2)[1]
listdata = []
for data in frozenlistdata:
    #   print(list(data), frozenlistdata[data])
    datatemp = [tuple((data)), frozenlistdata[data]]
    listdata.append(datatemp)

pprint.pprint((listdata))


# print(confidence(["BREAD", "COFFEE"], ["SUGER"], transactions))
# confidence(["BREAD"], ["SUGER", "COFFEE"], transactions)

# single items
# items = getItemList(transactions)

# print("ITEMS: ", countItemOccurence(items, transactions))
itemset = aprioriAlgorithm(transactions, 1)[1]
# pprint.pprint(itemset)

new_dict = {tuple(fset)[0]: itemset[fset] for fset in itemset}

# print(new_dict)
# pprint.pprint(aprioriAlgorithm(transactions,3)[0])
# print(confidence(["BREAD", "COFFEE"], ["SUGER"], transactions))


## Important Function ##
print(getItemList(transactions))
# print(aprioriAlgorithm(transactions, support_vector=2))




def updated_stock_data():
    
    all_accepted_itemset = aprioriAlgorithm(transactions, support_vector=2)
    
    def average_confidence(item, all_accepted_itemset):
        squared_confidence = 0
        total_items = 0
        for itemsetOfSameSize in all_accepted_itemset:
            frozenlistdata = itemsetOfSameSize
            for data in frozenlistdata:
                confid = confidence(list(data), item, transactions)
                if (confid>0):
                    squared_confidence += confid 
                    #print(data, confidence(list(data), item, transactions))
                    total_items += 1
        if total_items == 0:
            return 0
        else:
            return squared_confidence/total_items
        
    StockData = []
    
    for item in stock:
        avg_confidence = round(average_confidence([item[0]], all_accepted_itemset), 2)
        StockData.append((item[0], item[1], str(avg_confidence)+'%'))
